...
read_when:
  - You need detailed behavior for agentme onboard
...
This page is the full reference for `agentme onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.agentme/agentme.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `agentme doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `agentme pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `agentme health`.
    - `agentme status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.agentme/.env` so launchd can read it.
...
- OAuth credentials: `~/.agentme/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.agentme/agents/<agentId>/agent/auth-profiles.json`
...
- Command reference: [`agentme onboard`](/cli/onboard)
