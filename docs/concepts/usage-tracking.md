...
- `/usage cost` in chats: local cost summary aggregated from AgentMe session logs.
- CLI: `agentme status --usage` prints a full per-provider breakdown.
- CLI: `agentme channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
