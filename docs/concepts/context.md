...
read_when:
  - You want to understand what “context” means in AgentMe
...
“Context” is **everything AgentMe sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (AgentMe-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How AgentMe builds the system prompt

The system prompt is **AgentMe-owned** and rebuilt each run. It includes:
...
By default, AgentMe injects a fixed set of workspace files (if present):
