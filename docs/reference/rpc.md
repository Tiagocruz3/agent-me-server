...
AgentMe integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- AgentMe owns lifecycle when `channels.signal.autoStart=true`.
...
- AgentMe spawns `imsg rpc` as a child process (legacy iMessage integration).
