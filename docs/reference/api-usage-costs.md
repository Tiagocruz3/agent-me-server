...
AgentMe features that can generate provider usage or paid API calls.
...
- `agentme status --usage` and `agentme channels list` show provider **usage windows**
...
AgentMe can pick up credentials from:
...
- `agentme status --usage`
- `agentme models status --json`
...
`agentme models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
