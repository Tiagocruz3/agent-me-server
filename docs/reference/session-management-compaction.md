...
This document explains how AgentMe manages sessions end-to-end:
...
AgentMe is designed around a single **Gateway process** that owns session state.
...
AgentMe persists sessions in two layers:
...
- Transcripts: `~/.agentme/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

AgentMe resolves these via `src/config/sessions.ts`.
...
AgentMe intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (AgentMe consumes the events, but Pi decides when to compact).
...
AgentMe also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, AgentMe bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, AgentMe leaves it alone.
...
- `agentme status` (CLI)
- `agentme sessions` / `sessions --json`
...
AgentMe supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- AgentMe strips/suppresses this in the delivery layer.

As of `2026.1.10`, AgentMe also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
AgentMe uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but AgentMe’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `agentme status`.
