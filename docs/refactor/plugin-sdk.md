...
- Publish as `agentme/plugin-sdk` (or export from core under `agentme/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `AgentMePluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: AgentMeConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: AgentMeConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: AgentMeConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: AgentMeConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: AgentMeConfig): string;
  };
};
```
...
- Introduce `agentme/plugin-sdk`.
- Add `api.runtime` to `AgentMePluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `agentmeRuntime: ">=2026.2.0"`).
