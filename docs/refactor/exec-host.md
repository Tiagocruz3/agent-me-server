...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.agentme/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.agentme/node.json` (node id + pairing token).
...
Path: `~/.agentme/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.agentme/exec-approvals.sock",
...
- Unix socket at `~/.agentme/exec-approvals.sock` (0600).
