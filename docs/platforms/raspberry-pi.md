---
summary: "AgentMe on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up AgentMe on a Raspberry Pi
  - Running AgentMe on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# AgentMe on Raspberry Pi

## Goal

Run a persistent, always-on AgentMe Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most AgentMe features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled agentme
...
```bash
# Check logs
journalctl -u agentme --no-pager -n 100

# Common fix: rebuild
cd ~/agentme  # if using hackable install
npm run build
sudo systemctl restart agentme
```
