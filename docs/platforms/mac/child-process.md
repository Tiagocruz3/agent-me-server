...
service via the external `agentme` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`AGENTME_PROFILE`; legacy `com.agentme.*` is supported).
