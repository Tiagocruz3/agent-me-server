...
read_when:
  - You want AgentMe to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables AgentMe to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`agentme onboard`) and `agentme channels add` list optional channel plugins.
...
```bash
agentme plugins install @agentme/nostr
```

Use a local checkout (dev workflows):

```bash
agentme plugins install --link <path-to-agentme>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "agentme@example.com",
        "lud16": "agentme@example.com"
      }
    }
  }
}
