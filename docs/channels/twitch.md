...
read_when:
  - Setting up Twitch chat integration for AgentMe
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. AgentMe connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
agentme plugins install @agentme/twitch
```

Local checkout (when running from a git repo):

```bash
agentme plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `AGENTME_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "agentme", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use AGENTME_TWITCH_ACCESS_TOKEN env var)
...
```bash
AGENTME_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
