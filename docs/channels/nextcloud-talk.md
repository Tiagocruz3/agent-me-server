...
```bash
agentme plugins install @agentme/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
agentme plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "AgentMe" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `agentme pairing list nextcloud-talk`
  - `agentme pairing approve nextcloud-talk <CODE>`
