...
Matrix is an open, decentralized messaging protocol. AgentMe connects as a Matrix **user**
...
```bash
agentme plugins install @agentme/matrix
```

Local checkout (when running from a git repo):

```bash
agentme plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `agentme plugins install @agentme/matrix`
   - From a local checkout: `agentme plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: AgentMe calls the same
     login endpoint, stores the access token in `~/.agentme/credentials/matrix/credentials.json`,
...
- On first connection, AgentMe requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  AgentMe logs a warning.
...
`~/.agentme/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `agentme pairing list matrix`
  - `agentme pairing approve matrix <CODE>`
...
- On startup, AgentMe resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
