...
Tlon is a decentralized messenger built on Urbit. AgentMe connects to your Urbit ship and can
...
```bash
agentme plugins install @agentme/tlon
```

Local checkout (when running from a git repo):

```bash
agentme plugins install ./extensions/tlon
```
...
Use these with `agentme message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, AgentMe replies in-thread.
