...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects AgentMe to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
agentme plugins install @agentme/feishu
```

Local checkout (when running from a git repo):

```bash
agentme plugins install ./extensions/feishu
```
...
If you just installed AgentMe, run the wizard:
...
2. Configuring app credentials in AgentMe
...
1. You already ran `agentme channels add` for Feishu
2. The gateway is running (`agentme gateway status`)
...
```bash
agentme pairing approve feishu <CODE>
```
...
2. Run `agentme logs --follow` and look for `chat_id`
...
2. Run `agentme logs --follow` and look for `open_id`
...
| `agentme gateway restart` | Restart gateway service       |
| `agentme logs --follow`   | Tail gateway logs             |
...
4. Check logs: `agentme logs --follow`
...
5. Ensure the gateway is running: `agentme gateway status`
6. Check logs: `agentme logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.agentme/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.agentme/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
