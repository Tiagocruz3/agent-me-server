...
```bash
agentme plugins install @agentme/msteams
```

Local checkout (when running from a git repo):

```bash
agentme plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
AgentMe will offer the local install path automatically.
...
3. Configure AgentMe with those credentials.
...
- Talk to AgentMe via Teams DMs, group chats, or channels.
...
- On startup, AgentMe resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.agentme/agentme.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring AgentMe, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                   |
   | ------------------ | ------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `agentme-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `agentme plugins install @agentme/msteams`
   - From a local checkout: `agentme plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "AgentMe in Teams", "full": "AgentMe in Teams" },
...
AgentMe handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, AgentMe only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/AgentMeShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

AgentMe sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `agentme message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.agentme/msteams-polls.json`.
...
```bash
agentme message send --channel msteams \
...
agentme message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
agentme message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
agentme message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
agentme message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
