...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/agentme`)
...
If the page fails to load, start the Gateway first: `agentme gateway`.
...
you revoke it with `agentme devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.agentme/agentme.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. AgentMe
...
AgentMe **blocks** Control UI connections without device identity.
