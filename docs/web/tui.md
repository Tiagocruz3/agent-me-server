...
```bash
agentme tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `agentme tui --deliver`
...
- Check the Gateway logs: `agentme logs --follow`.
- Confirm the agent can run: `agentme status` and `agentme models status`.
...
- No agents in picker: check `agentme agents list` and your routing config.
