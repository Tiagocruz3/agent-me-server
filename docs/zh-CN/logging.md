...
你可以在 `~/.agentme/agentme.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/agentme.log"
  }
}
```
...
```bash
agentme channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.agentme/agentme.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/agentme/agentme-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- AgentMe 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
AGENTME_DIAGNOSTICS=telegram.http,telegram.payload
```
...
- 你也可以使用 `agentme plugins enable diagnostics-otel` 启用插件。
...
- `agentme.tokens`（计数器，属性：`agentme.token`、`agentme.channel`、`agentme.provider`、`agentme.model`）
- `agentme.cost.usd`（计数器，属性：`agentme.channel`、`agentme.provider`、`agentme.model`）
- `agentme.run.duration_ms`（直方图，属性：`agentme.channel`、`agentme.provider`、`agentme.model`）
- `agentme.context.tokens`（直方图，属性：`agentme.context`、`agentme.channel`、`agentme.provider`、`agentme.model`）

消息流：

- `agentme.webhook.received`（计数器，属性：`agentme.channel`、`agentme.webhook`）
- `agentme.webhook.error`（计数器，属性：`agentme.channel`、`agentme.webhook`）
- `agentme.webhook.duration_ms`（直方图，属性：`agentme.channel`、`agentme.webhook`）
- `agentme.message.queued`（计数器，属性：`agentme.channel`、`agentme.source`）
- `agentme.message.processed`（计数器，属性：`agentme.channel`、`agentme.outcome`）
- `agentme.message.duration_ms`（直方图，属性：`agentme.channel`、`agentme.outcome`）

队列 + 会话：

- `agentme.queue.lane.enqueue`（计数器，属性：`agentme.lane`）
- `agentme.queue.lane.dequeue`（计数器，属性：`agentme.lane`）
- `agentme.queue.depth`（直方图，属性：`agentme.lane` 或 `agentme.channel=heartbeat`）
- `agentme.queue.wait_ms`（直方图，属性：`agentme.lane`）
- `agentme.session.state`（计数器，属性：`agentme.state`、`agentme.reason`）
- `agentme.session.stuck`（计数器，属性：`agentme.state`）
- `agentme.session.stuck_age_ms`（直方图，属性：`agentme.state`）
- `agentme.run.attempt`（计数器，属性：`agentme.attempt`）

### 导出的 span（名称 + 关键属性）

- `agentme.model.usage`
  - `agentme.channel`、`agentme.provider`、`agentme.model`
  - `agentme.sessionKey`、`agentme.sessionId`
  - `agentme.tokens.*`（input/output/cache_read/cache_write/total）
- `agentme.webhook.processed`
  - `agentme.channel`、`agentme.webhook`、`agentme.chatId`
- `agentme.webhook.error`
  - `agentme.channel`、`agentme.webhook`、`agentme.chatId`、`agentme.error`
- `agentme.message.processed`
  - `agentme.channel`、`agentme.outcome`、`agentme.chatId`、`agentme.messageId`、`agentme.sessionKey`、`agentme.sessionId`、`agentme.reason`
- `agentme.session.stuck`
  - `agentme.state`、`agentme.ageMs`、`agentme.queueDepth`、`agentme.sessionKey`、`agentme.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `agentme doctor`。
