...
read_when:
  - 安装 AgentMe
  - 你想从 GitHub 安装
summary: 安装 AgentMe（推荐安装器、全局安装或从源代码安装）
title: 安装
x-i18n:
...
pnpm approve-builds -g                # 批准 agentme、node-llama-cpp、sharp 等
pnpm add -g agentme@latest           # 重新运行以执行 postinstall 脚本
```
...
提示：如果你还没有全局安装，请通过 `pnpm agentme ...` 运行仓库命令。
...
- 运行新手引导：`agentme onboard --install-daemon`
- 快速检查：`agentme doctor`
- 检查 Gateway 网关健康状态：`agentme status` + `agentme health`
- 打开仪表板：`agentme dashboard`
...
```bash
# 显式 npm
curl -fsSL https://agentme.ai/install.sh | bash -s -- --install-method npm
...
- `AGENTME_GIT_DIR=...`
- `AGENTME_GIT_UPDATE=0|1`
- `AGENTME_NO_PROMPT=1`
- `AGENTME_DRY_RUN=1`
- `AGENTME_NO_ONBOARD=1`
- `SHARP_IGNORE_GLOBAL_LIBVIPS=0|1`（默认：`1`；避免 `sharp` 针对系统 libvips 构建）

## 故障排除：找不到 `agentme`（PATH）
...
如果 `$(npm prefix -g)/bin`（macOS/Linux）或 `$(npm prefix -g)`（Windows）**不**在 `echo "$PATH"` 的输出中，你的 shell 无法找到全局 npm 二进制文件（包括 `agentme`）。
