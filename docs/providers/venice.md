---
summary: "Use Venice AI privacy-focused models in AgentMe"
read_when:
  - You want privacy-focused inference in AgentMe
...
```bash
agentme onboard --auth-choice venice-api-key
```
...
```bash
agentme chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, AgentMe shows all available Venice models. Pick based on your needs:
...
```bash
agentme models set venice/claude-opus-45
agentme models set venice/llama-3.3-70b
```

List all available models:

```bash
agentme models list | grep venice
```

## Configure via `agentme configure`

1. Run `agentme configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
AgentMe automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
agentme chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
agentme chat --model venice/claude-opus-45

# Use uncensored model
agentme chat --model venice/venice-uncensored

# Use vision model with image
agentme chat --model venice/qwen3-vl-235b-a22b

# Use coding model
agentme chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `agentme models list` to see currently available models. Some models may be temporarily offline.
