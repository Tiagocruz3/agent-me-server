---
summary: "Use MiniMax M2.1 in AgentMe"
read_when:
  - You want MiniMax models in AgentMe
...
```bash
agentme plugins enable minimax-portal-auth  # skip if already loaded.
agentme gateway restart  # restart if gateway is already running
agentme onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/agentme/agentme/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `agentme.json`:
...
- Use `agentme models list` and `agentme models set minimax/MiniMax-M2.1` to switch.
...
- Running `agentme configure` and selecting **MiniMax M2.1**, or
