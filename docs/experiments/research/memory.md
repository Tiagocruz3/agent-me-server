...
read_when:
  - Designing workspace memory (~/.agentme/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep AgentMe integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.agentme/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.agentme/workspace` as canonical human-readable memory.
...
```
~/.agentme/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in AgentMe**, but keep a separable core library.

### Why integrate into AgentMe?

- AgentMe already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `agentme memory recall "…" --k 25 --since 30d`
  - `agentme memory reflect --since 7d`
...
Pragmatic take for `~/.agentme/workspace`:
