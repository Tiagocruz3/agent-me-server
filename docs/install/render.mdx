...
Deploy AgentMe on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/agentme/agentme)
...
repository configures everything needed to run AgentMe:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: AGENTME_STATE_DIR
        value: /data/.agentme
      - key: AGENTME_WORKSPACE_DIR
        value: /data/workspace
      - key: AGENTME_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/agentme`.
...
If you use the original AgentMe repository, Render will not auto-deploy your AgentMe. To update it, run a manual Blueprint sync from the dashboard.
...
For AgentMe, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any AgentMe host.
