...
Deploy AgentMe on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/agentme` — Control UI
...
- `AGENTME_STATE_DIR=/data/.agentme` (recommended)
- `AGENTME_WORKSPACE_DIR=/data/workspace` (recommended)
- `AGENTME_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your AgentMe state + workspace so you can migrate to another host without losing config or memory.
