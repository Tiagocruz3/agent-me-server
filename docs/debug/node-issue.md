...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in AgentMe
...
Running AgentMe via Node with `tsx` fails at startup with:

```
[agentme] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `agentme status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch agentme.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node agentme.mjs status` works on Node 25.
