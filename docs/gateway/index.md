...
- Replaces the legacy `gateway` command. CLI entry point: `agentme gateway`.
...
agentme gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```

- Config hot reload watches `~/.agentme/agentme.json` (or `AGENTME_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__agentme__/canvas/` from `~/.agentme/workspace/canvas`. Disable with `canvasHost.enabled=false` or `AGENTME_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `AGENTME_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `AGENTME_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.agentme.*` may still exist)
- Linux: `agentme-gateway-<profile>.service`
- Windows: `AgentMe Gateway (<profile>)`

Install metadata is embedded in the service config:

- `AGENTME_SERVICE_MARKER=agentme`
- `AGENTME_SERVICE_KIND=gateway`
- `AGENTME_SERVICE_VERSION=<version>`
...
```bash
agentme --dev setup
agentme --dev gateway --allow-unconfigured
...
- `AGENTME_STATE_DIR=~/.agentme-dev`
- `AGENTME_CONFIG_PATH=~/.agentme-dev/agentme.json`
- `AGENTME_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.agentme/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `AGENTME_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `AGENTME_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `AGENTME_CONFIG_PATH`
- unique `AGENTME_STATE_DIR`
...
```bash
agentme --profile main gateway install
agentme --profile rescue gateway install
```

Example:

```bash
AGENTME_CONFIG_PATH=~/.agentme/a.json AGENTME_STATE_DIR=~/.agentme-a agentme gateway --port 19001
AGENTME_CONFIG_PATH=~/.agentme/b.json AGENTME_STATE_DIR=~/.agentme-b agentme gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `agentme health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `agentme`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `agentme gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.agentme.*` is cleaned up).
  - `agentme doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are AgentMe profile services.
  ...
  - Cleanup: `agentme gateway uninstall` (current service) and `agentme doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `agentme gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- AgentMe.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.agentme.*` labels still unload cleanly).
- To stop it cleanly, use `agentme gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `agentme gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `agentme gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

AgentMe installs a **systemd user service** by default on Linux/WSL2. We
...
`agentme gateway install` writes the user unit. `agentme doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/agentme-gateway[-<profile>].service`:

```
[Unit]
Description=AgentMe Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/agentme gateway --port 18789
Restart=always
RestartSec=5
Environment=AGENTME_GATEWAY_TOKEN=
...
```
systemctl --user enable --now agentme-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/agentme-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now agentme-gateway[-<profile>].service
```
...
- `agentme gateway health|status` — request health/status over the Gateway WS.
- `agentme message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `agentme agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `agentme gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `agentme gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `agentme gateway` and the legacy TCP control port.
