---
summary: "All configuration options for ~/.agentme/agentme.json with examples"
read_when:
...
AgentMe reads an optional **JSON5** config from `~/.agentme/agentme.json` (comments + trailing commas allowed).

If the file is missing, AgentMe uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.agentme/workspace`). You usually only need a config to:
...
AgentMe only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `agentme doctor`, `agentme logs`, `agentme health`, `agentme status`, `agentme service`, `agentme help`).
- Run `agentme doctor` to see the exact issues.
- Run `agentme doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `agentme config set`. Keep a backup of `~/.agentme/agentme.json`.
...
```bash
agentme gateway call config.get --params '{}' # capture payload.hash
agentme gateway call config.apply --params '{
...
```bash
agentme gateway call config.get --params '{}' # capture payload.hash
agentme gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.agentme/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@agentme", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.agentme/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.agentme/workspace" }],
}
```
...
{ "$include": "/etc/agentme/base.json5" } // absolute
...
```json5
// ~/.agentme/clients/mueller/agents.json5
[
...
```json5
// ~/.agentme/clients/mueller/broadcast.json5
{
...
AgentMe reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, AgentMe runs your login shell and imports only the missing expected keys (never overrides).
...
- `AGENTME_LOAD_SHELL_ENV=1`
- `AGENTME_SHELL_ENV_TIMEOUT_MS=15000`
...
AgentMe stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.agentme/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.agentme/credentials/oauth.json` (or `$AGENTME_STATE_DIR/credentials/oauth.json`)
...
- `~/.agentme/agent/*` (migrated by `agentme doctor` into `~/.agentme/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `AGENTME_OAUTH_DIR`
- Agent dir (default agent root override): `AGENTME_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, AgentMe imports `oauth.json` entries into `auth-profiles.json`.
...
If set, AgentMe derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/agentme/agentme-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/agentme/agentme.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.agentme/credentials/whatsapp/biz
          // authDir: "~/.agentme/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `agentme doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@agentme", "agentme"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@agentme"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.agentme/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.agentme/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.agentme/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.agentme/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.agentme/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.agentme/workspace-home" },
      { id: "work", workspace: "~/.agentme/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `agentme.json`).
...
AgentMe starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["agentme-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-agentme",
...
AgentMe starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `agentme plugins install @agentme/mattermost` (or `./extensions/mattermost` from a git checkout).
...
AgentMe starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
AgentMe spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, AgentMe
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/agentme" } },
}
```
...
When a file exceeds this limit, AgentMe logs a warning and injects a truncated
...
message envelopes). If unset, AgentMe uses the host timezone at runtime.
...
`[agentme]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[agentme]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[agentme]"`, AgentMe will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, AgentMe generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.agentme/settings/tts.json",
      elevenlabs: {
...
AgentMe also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, AgentMe currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `agentme configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:agentme`: all built-in AgentMe tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.agentme/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.agentme/sandboxes",
        docker: {
          image: "agentme-sandbox:bookworm-slim",
          containerPrefix: "agentme-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "agentme-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "agentme-sandbox-browser:bookworm-slim",
          containerPrefix: "agentme-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
AgentMe uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.agentme/agents/<agentId>/agent/models.json` or by defining the same schema inside your
AgentMe config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, AgentMe writes/merges a `models.json` into
`~/.agentme/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. AgentMe uses
...
- Shortcut: `agentme onboard --auth-choice opencode-zen`.
...
Shortcut: `agentme onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `agentme onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `agentme onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `agentme onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `agentme onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `agentme onboard --auth-choice minimax-api`.
...
- Override the agent config root with `AGENTME_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.agentme/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.agentme/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.agentme/agents/{agentId}/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
...
- `resetByType`: per-session overrides for `direct`, `group`, and `thread`. Legacy `dm` key is accepted as an alias for `direct`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, AgentMe stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.agentme/skills` (workspace skills
...
from `~/.agentme/extensions`, `<workspace>/.agentme/extensions`, plus any
...
### `browser` (agentme-managed browser)

AgentMe can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for agentme and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (AgentMe.app menubar, or `agentme gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      agentme: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/agentme" }
...
- Examples: `"/ui"`, `"/agentme"`, `"/apps/agentme"`.
...
- When a connection comes from one of these IPs, AgentMe uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `agentme gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `AGENTME_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `AGENTME_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. AgentMe
...
- AgentMe.app watches `~/.agentme/agentme.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.agentme/agentme.json` (or `AGENTME_CONFIG_PATH`) and applies changes automatically.
...
- `~/.agentme/agentme.json` (or `AGENTME_CONFIG_PATH`)
...
- `AGENTME_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `agentme --dev …` → uses `~/.agentme-dev` + shifts ports from base `19001`
- `agentme --profile <name> …` → uses `~/.agentme-<name>` (port via config/env/flags)
...
```bash
AGENTME_CONFIG_PATH=~/.agentme/a.json \
AGENTME_STATE_DIR=~/.agentme-a \
agentme gateway --port 19001
```
...
Gmail helper config (used by `agentme webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "agentme@gmail.com",
...
- Set `AGENTME_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, AgentMe defaults `serve.path` to `/` so
...
Default root: `~/.agentme/workspace/canvas`  
Default port: `18793` (chosen to avoid the agentme browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__agentme__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__agentme__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
- env: `AGENTME_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.agentme/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.agentme/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.agentme/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_agentme-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `agentme` (advertises `agentme.local`). Override with `AGENTME_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_agentme-gw._tcp` under `~/.agentme/dns/` using the configured discovery domain (example: `agentme.internal.`).
