...
- During heartbeat runs, AgentMe treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `agentme sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, AgentMe skips the heartbeat run entirely (no model call).
...
```bash
agentme system event --text "Check for urgent follow-ups" --mode now
```
