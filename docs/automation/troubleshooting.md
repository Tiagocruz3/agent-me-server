...
agentme system heartbeat last
agentme logs --follow
agentme config get agents.defaults.heartbeat
agentme channels status --probe
```
...
```bash
agentme config get agents.defaults.heartbeat.activeHours
agentme config get agents.defaults.heartbeat.activeHours.timezone
agentme config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
agentme cron list
agentme logs --follow
```
