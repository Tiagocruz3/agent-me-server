...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `agentme sandbox explain`.
...
Legacy `agent.*` configs are migrated by `agentme doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=agentme-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${AGENTME_STATE_DIR:-$HOME/.agentme}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
