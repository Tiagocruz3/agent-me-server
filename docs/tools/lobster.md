---
title: Lobster
summary: "Typed workflow runtime for AgentMe with resumable approval gates."
description: Typed workflow runtime for AgentMe — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets AgentMe run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: AgentMe runs one Lobster tool call and gets a structured result.
...
AgentMe launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | agentme.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
agentme.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In AgentMe tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the AgentMe Gateway (see the [Lobster repo](https://github.com/agentme/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), AgentMe keeps core tools enabled. To restrict core
...
→ agentme calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ agentme calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls AgentMe tools that do.
