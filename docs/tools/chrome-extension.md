---
summary: "Chrome extension: let AgentMe drive your existing Chrome tab"
read_when:
...
The AgentMe Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate agentme-managed Chrome profile.
...
AgentMe then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the AgentMe release (npm package) as static files. There is no separate “build” step.

After upgrading AgentMe:

- Re-run `agentme browser extension install` to refresh the installed files under your AgentMe state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

AgentMe ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want AgentMe to control.
...
- `ON`: attached; AgentMe can drive that tab.
...
Debugging: `agentme sandbox explain`
...
`agentme browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `agentme browser extension install` first to copy the extension to a stable location under your AgentMe state directory.
...
- **This is not isolated** like the dedicated agentme-managed profile.
