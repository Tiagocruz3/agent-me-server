---
summary: "Agent tool surface for AgentMe (browser, canvas, nodes, message, cron) replacing legacy `agentme-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `agentme-*` skills
title: "Tools"
---

# Tools (AgentMe)

AgentMe exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `agentme-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `agentme.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, AgentMe logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`agentme node run`).
...
- Requires a Brave API key (recommended: `agentme configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated AgentMe-managed browser.
...
- Quick smoke: `agentme nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `agentme gateway` restart in-place)
