...
- **gateway host** → `agentme` process on the gateway machine
...
`~/.agentme/exec-approvals.json` directly.

CLI: `agentme approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
