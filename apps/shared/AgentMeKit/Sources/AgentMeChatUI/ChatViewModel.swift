...
private let chatUILogger = Logger(subsystem: "ai.agentme", category: "AgentMeChatUI")

@MainActor
@Observable
public final class AgentMeChatViewModel {
    public private(set) var messages: [AgentMeChatMessage] = []
    ...
    public var attachments: [AgentMePendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [AgentMeChatPendingToolCall] = []
    public private(set) var sessions: [AgentMeChatSessionEntry] = []
    private let transport: any AgentMeChatTransport
    ...
    private var pendingToolCallsById: [String: AgentMeChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any AgentMeChatTransport) {
    ...
    public var sessionChoices: [AgentMeChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [AgentMeChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [AgentMeChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: AgentMePendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [AgentMeChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: AgentMeChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [AgentMeChatMessage]) -> [AgentMeChatMessage] {
        var result: [AgentMeChatMessage] = []
    ...
    private static func dedupeKey(for message: AgentMeChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [AgentMeChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> AgentMeChatAttachmentPayload in
            AgentMeChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> AgentMeChatSessionEntry {
        AgentMeChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: AgentMeChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: AgentMeChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: AgentMeAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = AgentMeChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> AgentMePlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
