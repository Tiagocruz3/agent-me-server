...
@MainActor
struct AgentMeChatComposer: View {
    @Bindable var viewModel: AgentMeChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(AgentMeChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(AgentMeChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \AgentMePendingAttachment.id)
                { (att: AgentMePendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            AgentMePlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(AgentMeChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(AgentMeChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(AgentMeChatTheme.subtleCard)
        .clipShape(Capsule())
    }
