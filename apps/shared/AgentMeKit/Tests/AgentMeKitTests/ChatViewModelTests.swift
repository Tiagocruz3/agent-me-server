...
private final class TestChatTransport: @unchecked Sendable, AgentMeChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [AgentMeChatHistoryPayload]
    private let sessionsResponses: [AgentMeChatSessionsListResponse]

    private let stream: AsyncStream<AgentMeChatTransportEvent>
    private let continuation: AsyncStream<AgentMeChatTransportEvent>.Continuation

    init(
        historyResponses: [AgentMeChatHistoryPayload],
        sessionsResponses: [AgentMeChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<AgentMeChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> AgentMeChatHistoryPayload {
        ...
        return self.historyResponses.last ?? AgentMeChatHistoryPayload(
        ...
        attachments _: [AgentMeChatAttachmentPayload]) async throws -> AgentMeChatSendResponse
    {
        ...
        return AgentMeChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> AgentMeChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? AgentMeChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: AgentMeChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = AgentMeChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = AgentMeChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { AgentMeChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = AgentMeChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = AgentMeChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = AgentMeChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { AgentMeChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
