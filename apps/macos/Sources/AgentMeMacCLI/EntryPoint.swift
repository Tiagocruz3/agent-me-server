...
struct AgentMeMacCLI {
    static func main() async {
        ...
        default:
            fputs("agentme-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      agentme-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      agentme-mac discover [--timeout <ms>] [--json] [--include-local]
      agentme-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      agentme-mac connect
      agentme-mac connect --url ws://127.0.0.1:18789 --json
      agentme-mac discover --timeout 3000 --json
      agentme-mac wizard --mode local
    """)
}
