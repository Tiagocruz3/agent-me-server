...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.agentme", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: AgentMePeekabooBridgeServices?
    private static var agentmeSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("AgentMe", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = AgentMePeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.agentmeSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.agentmeSocketPath, privacy: .public)")
    }
...
@MainActor
private final class AgentMePeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.agentme.peekaboo")
