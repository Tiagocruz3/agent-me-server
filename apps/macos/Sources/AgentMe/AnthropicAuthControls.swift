...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: AgentMeOAuthStore.AnthropicOAuthStatus = AgentMeOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: AgentMeOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = AgentMeOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = AgentMeOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try AgentMeOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. AgentMe can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: AgentMeOAuthStore.AnthropicOAuthStatus,
