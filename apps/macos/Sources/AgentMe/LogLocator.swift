...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["AGENTME_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/agentme")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("agentme-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("agentme-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/agentme/ (rolling or stdout), or nil if none exist.
