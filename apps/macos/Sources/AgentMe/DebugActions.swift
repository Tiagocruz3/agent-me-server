...
enum DebugActions {
    private static let verboseDefaultsKey = "agentme.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "agentme.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = AgentMePaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "AgentMe", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = AgentMePaths.configURL
        guard
