...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.agentme/agentme.json. Prefer AGENTME_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.agentme/agentme.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.agentme/agentme.json. Restart the gateway to apply."
        }
