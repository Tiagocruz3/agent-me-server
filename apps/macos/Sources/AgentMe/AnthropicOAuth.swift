...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (AgentMe token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: AgentMeOAuthStore.AnthropicOAuthStatus = AgentMeOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.agentme", category: "anthropic-oauth")
...
enum AgentMeOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let agentmeOAuthDirEnv = "AGENTME_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "AgentMe OAuth token file not found"
            case .unreadableFile: "AgentMe OAuth token file not readable"
            case .invalidJSON: "AgentMe OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in AgentMe OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "AgentMe OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.agentmeOAuthDirEnv]?
