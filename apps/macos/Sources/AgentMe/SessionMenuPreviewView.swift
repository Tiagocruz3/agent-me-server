...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.agentme", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> AgentMeSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: AgentMeSessionPreviewEntry,
    ...
    private static func cache(payload: AgentMeSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: AgentMeChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [AgentMeKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [AgentMeKit.AnyCodable]) -> [AgentMeChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(AgentMeChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: AgentMeChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: AgentMeChatMessage) -> Bool {
    ...
    private static func toolNames(for message: AgentMeChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: AgentMeChatMessage) -> String? {
