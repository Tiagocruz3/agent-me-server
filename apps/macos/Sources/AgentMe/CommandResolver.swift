...
enum CommandResolver {
    private static let projectRootDefaultsKey = "agentme.gatewayProjectRootPath"
    private static let helperName = "agentme"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let agentmeEntry = root.appendingPathComponent("agentme.mjs").path
        if FileManager().isReadableFile(atPath: agentmeEntry) { return agentmeEntry }
        let binEntry = root.appendingPathComponent("bin/agentme.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/agentme")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let agentmePaths = self.agentmeManagedPaths(home: home)
        if !agentmePaths.isEmpty {
            extras.insert(contentsOf: agentmePaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + agentmePaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func agentmeManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".agentme"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func agentmeExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectAgentMeExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("agentme.mjs").path,
            root.appendingPathComponent("bin/agentme.js").path,
        ]
    ...
    static func hasAnyAgentMeInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.agentmeExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func agentmeNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let agentmePath = self.projectAgentMeExecutable(projectRoot: root) {
                return [agentmePath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "agentme", subcommand] + extraArgs
            }
            if let agentmePath = self.agentmeExecutable(searchPaths: searchPaths) {
                return [agentmePath, subcommand] + extraArgs
            }

            let missingEntry = """
            agentme entrypoint missing (looked for dist/index.js or agentme.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real agentme CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/agentme"
        ...
        \(cliSection)
        \(projectSection)
        if command -v agentme >/dev/null 2>&1; then
          CLI="$(command -v agentme)"
          agentme \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/agentme.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/agentme.mjs"
            node "$PRJ/agentme.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/agentme.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/agentme.js"
            node "$PRJ/bin/agentme.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent agentme"
          pnpm --silent agentme \(quotedArgs);
        else
          echo "agentme CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? AgentMeConfigFile.loadDict()
