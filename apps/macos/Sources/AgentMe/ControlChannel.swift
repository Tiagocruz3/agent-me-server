...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: AgentMeProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.agentme", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: AgentMeKit.AnyCodable]()) {
                $0[$1.key] = AgentMeKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: AgentMeProtocol.AnyCodable?) -> [String: AgentMeProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: AgentMeProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: AgentMeKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: AgentMeProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("agentme.control.heartbeat")
    static let controlAgentEvent = Notification.Name("agentme.control.agent")
}
