...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.agentme.mac`) even if the shipped
    ...
    static func resolveNixMode(
        ...
        if environment["AGENTME_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "agentme.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "agentme.nixMode") { return true }
