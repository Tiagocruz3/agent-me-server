...
enum AgentMeLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = AgentMeOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = AgentMeFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        AgentMeLogging.bootstrapIfNeeded()
        let label = AgentMeLogging.makeLabel(subsystem: subsystem, category: category)
...
struct AgentMeOSLogHandler: LogHandler {
...
struct AgentMeFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = AgentMeLogging.parseLabel(self.label)
