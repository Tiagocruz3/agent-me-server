...
private let webChatSwiftLogger = Logger(subsystem: "ai.agentme", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: AgentMeChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> AgentMeChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(AgentMeChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> AgentMeChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                AgentMeGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<AgentMeChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any AgentMeChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = AgentMeChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: AgentMeChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<AgentMeChatView>) -> NSViewController
    {
