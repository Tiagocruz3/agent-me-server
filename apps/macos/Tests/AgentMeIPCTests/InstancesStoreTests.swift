...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: AgentMeProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: AgentMeProtocol.AnyCodable] = [
            "presence": .init([AgentMeProtocol.AnyCodable(entry)]),
        ]
        let payload = AgentMeProtocol.AnyCodable(payloadMap)
