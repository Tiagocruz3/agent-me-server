...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: AgentMeProtocol.AnyCodable(["ok": AgentMeProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: AgentMeProtocol.AnyCodable(["ok": AgentMeProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = AgentMeProtocol.AnyCodable([
            "runId": AgentMeProtocol.AnyCodable("run-1"),
            "sessionKey": AgentMeProtocol.AnyCodable("main"),
            "state": AgentMeProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: AgentMeProtocol.AnyCodable(["a": AgentMeProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
