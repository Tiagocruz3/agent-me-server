...
final class MotionService: MotionServicing {
    func activities(params: AgentMeMotionActivityParams) async throws -> AgentMeMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[AgentMeMotionActivityEntry], Error>) in
        ...
        return AgentMeMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: AgentMePedometerParams) async throws -> AgentMePedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<AgentMePedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = AgentMePedometerPayload(
