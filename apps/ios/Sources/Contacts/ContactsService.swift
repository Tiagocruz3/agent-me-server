...
final class ContactsService: ContactsServicing {
    ...
    func search(params: AgentMeContactsSearchParams) async throws -> AgentMeContactsSearchPayload {
        ...
        return AgentMeContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: AgentMeContactsAddParams) async throws -> AgentMeContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return AgentMeContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return AgentMeContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> AgentMeContactPayload {
        AgentMeContactPayload(
