...
final class RemindersService: RemindersServicing {
    func list(params: AgentMeRemindersListParams) async throws -> AgentMeRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[AgentMeReminderPayload], Error>) in
        ...
        return AgentMeRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: AgentMeRemindersAddParams) async throws -> AgentMeRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = AgentMeReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return AgentMeRemindersAddPayload(reminder: payload)
    }
