...
struct IOSGatewayChatTransport: AgentMeChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> AgentMeChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(AgentMeChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> AgentMeChatHistoryPayload {
        ...
        return try JSONDecoder().decode(AgentMeChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [AgentMeChatAttachmentPayload]) async throws -> AgentMeChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [AgentMeChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(AgentMeChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(AgentMeGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AgentMeGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AgentMeChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AgentMeAgentEventPayload.self)
                        {
