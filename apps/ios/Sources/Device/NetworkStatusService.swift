...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> AgentMeNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> AgentMeNetworkStatusPayload {
        let status: AgentMeNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> AgentMeNetworkStatusPayload {
        AgentMeNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
