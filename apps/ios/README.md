...
This is an **alpha** iOS app that connects to an AgentMe Gateway as a `role: node`.
...
3. On iOS: AgentMe → Settings → Gateway → paste setup code → Connect
...
cd apps/ios
xcodegen generate
xcodebuild test -project AgentMe.xcodeproj -scheme AgentMe -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/AgentMeKit` contains the shared transport/types used by the iOS app.
