---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for AgentMe deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, AgentMe cron scheduling for periodic checks, or version status checks on a machine running AgentMe (laptop, workstation, Pi, VPS).
---

# AgentMe Host Hardening

## Overview

Assess and harden the host running AgentMe, then align it to a user-defined risk tolerance without breaking access. Use AgentMe security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim AgentMe changes the host firewall, SSH, or OS updates; it does not.
...
5. AgentMe gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if AgentMe can access sensitive data.
...
### 2) Run AgentMe security audits (read-only)

As part of the default read-only checks, run `agentme security audit --deep`. Only offer alternatives if the user requests them:

1. `agentme security audit` (faster, non-probing)
2. `agentme security audit --json` (structured output)

Offer to apply AgentMe safe defaults (numbered):

1. `agentme security audit --fix`

Be explicit that `--fix` only tightens AgentMe defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check AgentMe version/update status (read-only)

As part of the default read-only checks, run `agentme update status`.
...
- Credential hygiene notes (location of AgentMe creds, prefer disk encryption)
...
After AgentMe install or first hardening pass, run at least one baseline audit and version check:

- `agentme security audit`
- `agentme security audit --deep`
- `agentme update status`

Ongoing monitoring is recommended. Use the AgentMe cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `agentme cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `agentme update status`
...
Before creating, `agentme cron list` and match on exact `name`. If found, `agentme cron edit <id> ...`.
If not found, `agentme cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `agentme update status` (preferred for source checkouts and channels)
2. `npm view agentme version` (published npm version)

## AgentMe command accuracy

Use only supported commands and flags:

- `agentme security audit [--deep] [--fix] [--json]`
- `agentme status` / `agentme status --deep`
- `agentme health --json`
- `agentme update status`
- `agentme cron add|list|runs|run`

Do not invent CLI flags or imply AgentMe enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by AgentMe compaction:
