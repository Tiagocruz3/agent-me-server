...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("AGENTME_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.AGENTME_STATE_DIR = path.join(base, ".agentme");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "agentme-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".agentme", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
