...
import type { AgentMeConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("agentme-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("agentme-bin-sherpa-");
      const modelDir = await makeTempDir("agentme-sherpa-model-");
      ...
      const cfg: AgentMeConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("agentme-bin-whispercpp-");
      const modelDir = await makeTempDir("agentme-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("agentme-bin-gemini-");
      ...
      const cfg: AgentMeConfig = { tools: { media: { image: {} } } };
