...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `agentme-e2e-${name}-`));
  const configDir = path.join(homeDir, ".agentme");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          AGENTME_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        AGENTME_GATEWAY_TOKEN: inst.gatewayToken,
        AGENTME_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          AGENTME_GATEWAY_PORT: String(gwA.port),
          AGENTME_GATEWAY_TOKEN: gwA.gatewayToken,
          AGENTME_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          AGENTME_GATEWAY_PORT: String(gwB.port),
          AGENTME_GATEWAY_TOKEN: gwB.gatewayToken,
          AGENTME_GATEWAY_PASSWORD: "",
        }),
