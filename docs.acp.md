...
This document describes how the AgentMe ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`agentme acp` exposes an ACP agent over stdio and forwards prompts to a running
AgentMe Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a AgentMe Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `agentme acp` over stdio.

Example config:

```bash
agentme config set gateway.remote.url wss://gateway-host:18789
agentme config set gateway.remote.token <token>
```

Example run:

```bash
agentme acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
agentme acp --session agent:main:main
agentme acp --session agent:design:main
agentme acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “AgentMe ACP” to start a thread.

## Execution Model

- ACP client spawns `agentme acp` and speaks ACP messages over stdio.
...
```bash
agentme acp --session agent:main:main
agentme acp --session-label "support inbox"
agentme acp --reset-session
```
...
`agentme acp` resolves the Gateway URL and auth from CLI flags or config:
