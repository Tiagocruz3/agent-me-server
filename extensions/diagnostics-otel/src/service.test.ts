...
vi.mock("agentme/plugin-sdk", async () => {
  const actual = await vi.importActual<typeof import("agentme/plugin-sdk")>("agentme/plugin-sdk");
  return {
    ...actual,
...
import { emitDiagnosticEvent } from "agentme/plugin-sdk";
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(
      telemetryState.histograms.get("agentme.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("agentme.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("agentme.webhook.processed");
    expect(spanNames).toContain("agentme.message.processed");
    expect(spanNames).toContain("agentme.session.stuck");
