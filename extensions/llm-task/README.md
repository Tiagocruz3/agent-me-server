...
`agentme.invoke --each`) without adding new AgentMe code per workflow.
...
This extension depends on AgentMe internal modules (the embedded agent runner).
It is intended to ship as a **bundled** AgentMe extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.agentme/extensions` as a standalone plugin directory.
