/**
 * Twitch channel plugin for AgentMe.
 *
...
import { buildChannelConfigSchema } from "agentme/plugin-sdk";
import type {
 ...
 * for AgentMe. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: AgentMeConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: AgentMeConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: AgentMeConfig): boolean => {
