import type { PluginRuntime } from "agentme/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.AGENTME_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "agentme-nostr-"));
  process.env.AGENTME_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.AGENTME_STATE_DIR?.trim() || env.AGENTME_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".agentme");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.AGENTME_STATE_DIR = previous;
    }
