...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as AgentMeConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as AgentMeConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as AgentMeConfig);
