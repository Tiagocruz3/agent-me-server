...
metadata: { "agentme": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == AgentMe `sessions_spawn`
- **File I/O** == AgentMe `read`/`write`
- **Remote fetch** == AgentMe `web_fetch` (or `exec` with curl when POST is required)
