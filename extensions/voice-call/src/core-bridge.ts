...
function resolveAgentMeRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.AGENTME_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set AGENTME_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveAgentMeRoot(), "dist", "extensionAPI.js");
