import type { AgentMePluginApi, AgentMePluginService } from "agentme/plugin-sdk";
...
function normalizeDenyList(cfg: AgentMePluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: AgentMePluginApi["config"]): string[] {
...
export default function register(api: AgentMePluginApi) {
  ...
  const timerService: AgentMePluginService = {
