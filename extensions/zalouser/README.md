...
AgentMe extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
agentme channels login --channel zalouser
...
```bash
agentme message send --channel zalouser --target <threadId> --message "Hello from AgentMe!"
```
...
```bash
agentme channels login --channel zalouser              # Login via QR
agentme channels login --channel zalouser --account work
agentme channels status --probe
agentme channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
agentme directory self --channel zalouser
agentme directory peers list --channel zalouser --query "name"
agentme directory groups list --channel zalouser --query "work"
agentme directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
agentme message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
agentme message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `agentme channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
agentme channels login --channel zalouser --account work
agentme message send --channel zalouser --account work --target <id> --message "Hello"
