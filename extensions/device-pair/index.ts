...
function resolveGatewayPort(cfg: AgentMePluginApi["config"]): number {
  const envRaw =
    process.env.AGENTME_GATEWAY_PORT?.trim() || process.env.CLAWDBOT_GATEWAY_PORT?.trim();
  if (envRaw) {
...
async function resolveTailnetHost(api: AgentMePluginApi): Promise<string | null> {
...
function resolveAuth(cfg: AgentMePluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: AgentMePluginApi): Promise<ResolveUrlResult> {
