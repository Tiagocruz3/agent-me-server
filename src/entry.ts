...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.AGENTME_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.AGENTME_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.AGENTME_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[agentme] ${parsed.error}`);
    process.exit(2);
  }
