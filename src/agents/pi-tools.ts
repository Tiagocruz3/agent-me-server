...
function resolveExecConfig(cfg: AgentMeConfig | undefined) {
...
export function createAgentMeCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      return [createAgentMeReadTool(freshReadTool)];
    }
