...
import { createAgentMeCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("agentme-ws-", async (workspaceDir) => {
      await withTempDir("agentme-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createAgentMeCodingTools({ workspaceDir });
    ...
    await withTempDir("agentme-ws-", async (workspaceDir) => {
      await withTempDir("agentme-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello agentme");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("defaults exec cwd to workspaceDir when workdir is omitted", async () => {
    await withTempDir("agentme-ws-", async (workspaceDir) => {
      const tools = createAgentMeCodingTools({ workspaceDir, exec: { host: "gateway" } });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("agentme-ws-", async (workspaceDir) => {
      await withTempDir("agentme-override-", async (overrideDir) => {
        const tools = createAgentMeCodingTools({ workspaceDir, exec: { host: "gateway" } });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("agentme-sandbox-", async (sandboxDir) => {
      await withTempDir("agentme-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "agentme-sbx-test",
        ...
        const tools = createAgentMeCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
