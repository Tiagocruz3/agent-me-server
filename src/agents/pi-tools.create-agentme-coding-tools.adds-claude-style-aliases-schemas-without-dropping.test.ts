...
import { __testing, createAgentMeCodingTools } from "./pi-tools.js";
...
describe("createAgentMeCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createAgentMeTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createAgentMeCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "agentme-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "agentme-outside.txt");
