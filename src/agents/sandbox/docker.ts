...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "agentme.sandbox=1");
  args.push("--label", `agentme.sessionKey=${params.scopeKey}`);
  args.push("--label", `agentme.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `agentme.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("agentme.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`agentme sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`agentme sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("agentme sandbox recreate --all");
}
