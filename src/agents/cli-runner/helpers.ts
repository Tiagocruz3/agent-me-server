...
/**
 * Cleanup suspended AgentMe CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: AgentMeConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "agentme-cli-images-"));
  const paths: string[] = [];
