...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "agentme-models-" });
}

const MODELS_CONFIG: AgentMeConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensureAgentMeModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureAgentMeModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureAgentMeModelsJson } = await import("./models-config.js");
      const { resolveAgentMeAgentDir } = await import("./agent-paths.js");

      await ensureAgentMeModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveAgentMeAgentDir(), "models.json");
