...
const defaultTools = createAgentMeCodingTools();

describe("createAgentMeCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "agentme-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    ...
    try {
      ...
      const contents = "Hello from agentme read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "agentme-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "agentme-workspace"),
    ...
    const tools = createAgentMeCodingTools({ sandbox });
