...
type WebSearchConfig = NonNullable<AgentMeConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: AgentMeConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("agentme configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.agentme.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://agentme.ai",
      "X-Title": "AgentMe Web Search",
    },
    body: JSON.stringify({
      model,
      messages: [
