...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "agentme-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "agentme-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "agentme-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "agentme-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.agentme when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "agentme-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers AGENTME_HOME over HOME", () => {
    vi.stubEnv("AGENTME_HOME", "/srv/agentme-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/agentme-home"));
...
describe("shortenHomePath", () => {
  it("uses $AGENTME_HOME prefix when AGENTME_HOME is set", () => {
    vi.stubEnv("AGENTME_HOME", "/srv/agentme-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/agentme-home")}/.agentme/agentme.json`)).toBe(
      "$AGENTME_HOME/.agentme/agentme.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $AGENTME_HOME replacement when AGENTME_HOME is set", () => {
    vi.stubEnv("AGENTME_HOME", "/srv/agentme-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/agentme-home")}/.agentme/agentme.json`),
    ).toBe("config: $AGENTME_HOME/.agentme/agentme.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/agentme")).toBe(path.resolve(os.homedir(), "agentme"));
  });
  ...
  it("prefers AGENTME_HOME for tilde expansion", () => {
    vi.stubEnv("AGENTME_HOME", "/srv/agentme-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/agentme")).toBe(path.resolve("/srv/agentme-home", "agentme"));
