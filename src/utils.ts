...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".agentme");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.AGENTME_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$AGENTME_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via AGENTME_STATE_DIR.
