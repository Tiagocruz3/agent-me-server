...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "AGENTME_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "agentme-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.AGENTME_STATE_DIR = path.join(home, ".agentme");
    ...
    await fs.mkdir(path.join(home, ".agentme"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.agentme${path.sep}media`);
