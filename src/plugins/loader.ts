...
import { discoverAgentMePlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: AgentMePluginDefinition;
  register?: AgentMePluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as AgentMePluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as AgentMePluginDefinition;
...
export function loadAgentMePlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverAgentMePlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "agentme/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: AgentMePluginModule | null = null;
    try {
      mod = jiti(candidate.source) as AgentMePluginModule;
    } catch (err) {
