...
import { loadAgentMePlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.AGENTME_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `agentme-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "agentme.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.AGENTME_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadAgentMePlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.AGENTME_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadAgentMePlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadAgentMePlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
