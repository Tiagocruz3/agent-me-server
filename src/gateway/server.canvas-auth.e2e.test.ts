...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.AGENTME_CONFIG_PATH;
  const prevDisableCache = process.env.AGENTME_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "agentme-canvas-auth-test-"));
  const configPath = path.join(dir, "agentme.json");
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.AGENTME_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.AGENTME_DISABLE_CONFIG_CACHE;
    } else {
      process.env.AGENTME_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
