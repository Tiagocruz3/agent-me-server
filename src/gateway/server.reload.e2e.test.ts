...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.AGENTME_SKIP_CHANNELS;
    prevSkipGmail = process.env.AGENTME_SKIP_GMAIL_WATCHER;
    process.env.AGENTME_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.AGENTME_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.AGENTME_SKIP_GMAIL_WATCHER;
    } else {
      process.env.AGENTME_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
