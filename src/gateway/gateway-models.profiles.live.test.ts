...
import type { AgentMeConfig, ModelProviderConfig } from "../config/types.js";
import { resolveAgentMeAgentDir } from "../agents/agent-paths.js";
...
import { ensureAgentMeModelsJson } from "../agents/models-config.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.AGENTME_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.AGENTME_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.AGENTME_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<AgentMeConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.AGENTME_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.AGENTME_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveAgentMeAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "agentme-live-state-"));
  process.env.AGENTME_STATE_DIR = tempStateDir;
  ...
  process.env.AGENTME_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.agentme-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveAgentMeAgentDir();
  const sanitizedCfg: AgentMeConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "AgentMe live tool probe (local, safe): " +
    ...
    process.env.AGENTME_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.AGENTME_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureAgentMeModelsJson(cfg);
      ...
      const rawModels = process.env.AGENTME_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.agentme-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
