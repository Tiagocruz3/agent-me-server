...
${formatHelpExamples([
  ['agentme agent --to +15555550123 --message "status update"', "Start a new session."],
  ['agentme agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'agentme agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'agentme agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['agentme agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'agentme agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.agentme.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.agentme.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['agentme agents set-identity --agent main --name "AgentMe" --emoji "🦞"', "Set name + emoji."],
  ["agentme agents set-identity --agent main --avatar avatars/agentme.png", "Set avatar path."],
  [
    "agentme agents set-identity --workspace ~/.agentme/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "agentme agents set-identity --identity-file ~/.agentme/workspace/IDENTITY.md --agent main",
