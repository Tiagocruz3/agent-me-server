...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("agentme node install"), formatCliCommand("agentme node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("agentme node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    AGENTME_LOG_PREFIX: baseEnv.AGENTME_LOG_PREFIX ?? "node",
