...
const STEP_LABELS: Record<string, string> = {
  ...
  "agentme doctor entry": "Checking doctor entrypoint",
  "agentme doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "agentme";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const AGENTME_REPO_URL = "https://github.com/agentme/agentme.git";
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("agentme@")) {
    return trimmed.slice("agentme@".length);
  }
...
async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "agentme.mjs");
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("agentme completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
function resolveGitInstallDir(): string {
  const override = process.env.AGENTME_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", AGENTME_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `AGENTME_GIT_DIR points at a non-git directory: ${params.dir}. Set AGENTME_GIT_DIR to an empty folder or an agentme checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`AGENTME_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveAgentMePackageRoot({
  ...
  defaultRuntime.log(theme.heading("AgentMe update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating AgentMe..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this AgentMe install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("agentme doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("agentme gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g agentme@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g agentme@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.AGENTME_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.AGENTME_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("agentme gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("agentme doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("agentme gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("agentme gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `agentme update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `AGENTME_GIT_DIR points at a non-git directory: ${gitDir}. Set AGENTME_GIT_DIR to an empty folder or an agentme checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via AGENTME_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update AgentMe to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["agentme update", "Update a source checkout (git)"],
        ["agentme update --channel beta", "Switch to beta channel (git + npm)"],
        ["agentme update --channel dev", "Switch to dev channel (git + npm)"],
        ["agentme update --tag beta", "One-off update to a dist-tag or version"],
        ["agentme update --no-restart", "Update without restarting the service"],
        ["agentme update --json", "Output result as JSON"],
        ["agentme update --yes", "Non-interactive (accept downgrade prompts)"],
        ["agentme update wizard", "Interactive update wizard"],
        ["agentme --update", "Shorthand for agentme update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run agentme update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.agentme.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["agentme update status", "Show channel + version status."],
          ["agentme update status --json", "JSON output."],
          ["agentme update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.agentme.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
