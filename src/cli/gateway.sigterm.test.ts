...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "agentme-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      AGENTME_NO_RESPAWN: "1",
      AGENTME_STATE_DIR: stateDir,
      AGENTME_SKIP_CHANNELS: "1",
      AGENTME_SKIP_GMAIL_WATCHER: "1",
      AGENTME_SKIP_CRON: "1",
      AGENTME_SKIP_BROWSER_CONTROL_SERVER: "1",
      AGENTME_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "agentme-entry-bootstrap.mjs");
