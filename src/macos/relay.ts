...
declare const __AGENTME_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __AGENTME_VERSION__ === "string" && __AGENTME_VERSION__) ||
  process.env.AGENTME_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  ...
  const { ensureAgentMeCliOnPath } = await import("../infra/path-env.js");
  ...
  process.on("uncaughtException", (error) => {
    console.error("[agentme] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
...
void main().catch((err) => {
  console.error("[agentme] Relay failed:", err instanceof Error ? (err.stack ?? err.message) : err);
  process.exit(1);
});
