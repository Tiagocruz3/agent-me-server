...
function setSlackDmPolicy(cfg: AgentMeConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "AgentMe";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for AgentMe`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: AgentMeConfig, allowFrom: string[]): AgentMeConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
