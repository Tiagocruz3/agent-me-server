...
`;
    ...
    expect(parsed.agentme.emoji).toBe("💾");
    expect(parsed.agentme.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.agentme.emoji).toBe("📝");
    expect(parsed.agentme.events).toEqual(["command"]);
    expect(parsed.agentme.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.agentme.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"agentme": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"agentme": {"events": ["test"]}}');
  });
...
describe("resolveAgentMeMetadata", () => {
  it("extracts agentme metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveAgentMeMetadata(frontmatter);
  ...
  it("returns undefined when agentme key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        agentme: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with AgentMe" },
            { id: "npm", kind: "npm", package: "@agentme/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@agentme/hook");
  });
    ...
    "agentme":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with AgentMe" }],
      },
  }
---
...
`;
    ...
    const agentme = resolveAgentMeMetadata(frontmatter);
    expect(agentme).toBeDefined();
    expect(agentme?.emoji).toBe("💾");
    expect(agentme?.events).toEqual(["command:new"]);
    expect(agentme?.requires?.config).toEqual(["workspace.dir"]);
    expect(agentme?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const agentme = resolveAgentMeMetadata(frontmatter);
    expect(agentme?.emoji).toBe("disk");
    expect(agentme?.events).toEqual(["command:new"]);
  });
});
