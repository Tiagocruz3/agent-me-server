...
This directory contains hooks that ship with AgentMe. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.agentme.ai/hooks/soul-evil
...
Hooks can be configured in `~/.agentme/agentme.json`:
...
2. Restart gateway: `pkill -9 -f 'agentme.*gateway' && pnpm agentme gateway`
3. Enable the hook: `agentme hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.agentme.ai/hooks
