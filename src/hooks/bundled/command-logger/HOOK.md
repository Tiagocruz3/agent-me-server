...
homepage: https://docs.agentme.ai/hooks#command-logger
metadata:
  {
    "agentme":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.agentme/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.agentme/logs/commands.log ~/.agentme/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/agentme`:
   ```
   /home/username/.agentme/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
