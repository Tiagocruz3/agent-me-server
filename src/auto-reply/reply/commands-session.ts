...
import { scheduleGatewaySigusr1Restart, triggerAgentMeRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting AgentMe in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerAgentMeRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting AgentMe via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
