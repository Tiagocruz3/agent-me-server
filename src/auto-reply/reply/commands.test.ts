...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "agentme-commands-"));
...
function buildParams(commandBody: string, cfg: AgentMeConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
