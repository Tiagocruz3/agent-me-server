...
import type { AgentMeConfig } from "../config/types.js";
import type {
...
export function isCommandEnabled(cfg: AgentMeConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: AgentMeConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as AgentMeConfig),
...
export function getCommandDetection(_cfg?: AgentMeConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: AgentMeConfig) {
