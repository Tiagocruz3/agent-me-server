...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.AGENTME_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*agentme-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "agentme";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "agentme");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "agentme-gw");
  ...
  records.push(`_agentme-gw._tcp IN PTR ${instanceLabel}._agentme-gw._tcp`);
  records.push(`${instanceLabel}._agentme-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._agentme-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; agentme-content-hash: ${contentHash}\n${contentBody}`;
}
