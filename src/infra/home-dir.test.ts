...
describe("resolveEffectiveHomeDir", () => {
  it("prefers AGENTME_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(path.resolve("/srv/agentme-home"));
  });
  ...
  it("expands AGENTME_HOME when set to ~", () => {
    const env = {
      AGENTME_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for AGENTME_HOME", () => {
    const result = resolveRequiredHomeDir(
      { AGENTME_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when AGENTME_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ AGENTME_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { AGENTME_HOME: "/srv/agentme-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/agentme-home")}/x`);
  });
