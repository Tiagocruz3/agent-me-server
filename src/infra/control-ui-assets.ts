...
import { resolveAgentMePackageRoot, resolveAgentMePackageRootSync } from "./agentme-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveAgentMePackageRoot({ argv1: normalized });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "agentme" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveAgentMePackageRootSync({
    argv1,
