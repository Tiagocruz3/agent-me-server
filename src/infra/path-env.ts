...
function candidateBinDirs(opts: EnsureAgentMePathOpts): string[] {
  ...
  // Bundled macOS app: `agentme` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "agentme");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/agentme` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "agentme"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `agentme` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureAgentMeCliOnPath(opts: EnsureAgentMePathOpts = {}) {
  if (isTruthyEnvValue(process.env.AGENTME_PATH_BOOTSTRAPPED)) {
    return;
  }
