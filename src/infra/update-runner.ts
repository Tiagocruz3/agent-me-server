...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "agentme-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "agentme.mjs");
    ...
    const doctorStep = await runStep(
      step("agentme doctor", doctorArgv, gitRoot, { AGENTME_UPDATE_IN_PROGRESS: "1" }),
    );
