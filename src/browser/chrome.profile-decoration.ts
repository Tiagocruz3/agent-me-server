...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".agentme-profile-decorated");
}
 ...
 */
export function decorateAgentMeProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_AGENTME_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_AGENTME_BROWSER_COLOR).toUpperCase();
