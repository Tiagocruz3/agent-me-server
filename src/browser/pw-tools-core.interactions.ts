  ...
  try {
    if (boxes.length > 0) {
      await page.evaluate((labels) => {
        const existing = document.querySelectorAll("[data-agentme-labels]");
        existing.forEach((el) => el.remove());

        const root = document.createElement("div");
        root.setAttribute("data-agentme-labels", "1");
        ...
        for (const label of labels) {
          ...
          box.setAttribute("data-agentme-labels", "1");
          ...
          tag.setAttribute("data-agentme-labels", "1");
