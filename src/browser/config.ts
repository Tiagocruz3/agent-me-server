import type { BrowserConfig, BrowserProfileConfig, AgentMeConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_AGENTME_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "agentme" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_AGENTME_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "agentme" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_AGENTME_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_AGENTME_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an AgentMe browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for agentme/agentme CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: AgentMeConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_AGENTME_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "agentme";
