...
// Mock loadConfig so resolveMaintenanceConfig() never reads a real agentme.json.
...
describe("rotateSessionFile", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "agentme-rotate-"));
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "agentme-pruning-integ-"));
    storePath = path.join(testDir, "sessions.json");
    savedCacheTtl = process.env.AGENTME_SESSION_CACHE_TTL_MS;
    process.env.AGENTME_SESSION_CACHE_TTL_MS = "0";
    clearSessionStoreCacheForTest();
  ...
  afterEach(async () => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.AGENTME_SESSION_CACHE_TTL_MS;
    } else {
      process.env.AGENTME_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
