...
const ETC_AGENTME_DIR = path.join(ROOT_DIR, "etc", "agentme");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "agentme.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcAgentMePath(...parts: string[]) {
  return path.join(ETC_AGENTME_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcAgentMePath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "agentme.json"))).toEqual({
      shared: true,
    });
  });
});
