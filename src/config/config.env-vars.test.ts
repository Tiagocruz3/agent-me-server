...
describe("config env vars", () => {
  ...
  it("loads ${VAR} substitutions from ~/.agentme/.env on repeated runtime loads", async () => {
    await withTempHome(async (home) => {
      await withEnvOverride(
        {
          AGENTME_STATE_DIR: path.join(home, ".agentme"),
          CLAWDBOT_STATE_DIR: undefined,
          AGENTME_HOME: undefined,
          CLAWDBOT_HOME: undefined,
          BRAVE_API_KEY: undefined,
          AGENTME_DISABLE_CONFIG_CACHE: "1",
        },
        async () => {
