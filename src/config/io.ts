...
import type { AgentMeConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
import { compareAgentMeVersions } from "./version.js";
...
function coerceConfig(value: unknown): AgentMeConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as AgentMeConfig;
}
...
function stampConfigVersion(cfg: AgentMeConfig): AgentMeConfig {
...
function warnIfConfigFromFuture(cfg: AgentMeConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareAgentMeVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer AgentMe (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: AgentMeConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): AgentMeConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as AgentMeConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as AgentMeConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: AgentMeConfig) {
    clearConfigCache();
...
// module scope. `AGENTME_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.AGENTME_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.AGENTME_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): AgentMeConfig {
...
export async function writeConfigFile(cfg: AgentMeConfig): Promise<void> {
  clearConfigCache();
