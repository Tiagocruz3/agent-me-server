...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "agentme-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".agentme", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<AgentMeConfig> = {},
): AgentMeConfig {
  const base: AgentMeConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "agentme"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".agentme", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
