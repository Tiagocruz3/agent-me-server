...
beforeAll(() => {
  previousProfile = process.env.AGENTME_PROFILE;
  process.env.AGENTME_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.AGENTME_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/agentme-root.js", () => ({
  resolveAgentMePackageRoot: vi.fn().mockResolvedValue("/tmp/agentme"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/agentme/pnpm-lock.yaml",
      markerPath: "/tmp/agentme/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("AgentMe status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("agentme status --all") ||
          l.includes("agentme --profile isolated status --all") ||
          l.includes("agentme status --all") ||
          l.includes("agentme --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.AGENTME_GATEWAY_TOKEN = "abcd1234";
    try {
