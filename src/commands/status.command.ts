...
export async function statusCommand(
  ...
  runtime.log(theme.heading("AgentMe status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("agentme security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("agentme security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.agentme.ai/faq");
  runtime.log("Troubleshooting: https://docs.agentme.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("agentme status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("agentme logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("agentme status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("agentme gateway probe")}`);
  }
}
