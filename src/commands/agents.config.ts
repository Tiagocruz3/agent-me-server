...
export function listAgentEntries(cfg: AgentMeConfig): AgentEntry[] {
...
function resolveAgentName(cfg: AgentMeConfig, agentId: string) {
...
function resolveAgentModel(cfg: AgentMeConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: AgentMeConfig): AgentSummary[] {
