...
function resolveMode(cfg: AgentMeConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("AgentMe doctor");

  const root = await resolveAgentMePackageRoot({
  ...
  let cfg: AgentMeConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("agentme configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("agentme config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("agentme setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("agentme doctor --fix")}" to apply changes.`);
  }
