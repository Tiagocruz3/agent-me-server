...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAgentMeAgentDir: vi.fn().mockReturnValue("/tmp/agentme-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/agentme-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/agentme-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveAgentMeAgentDir: mocks.resolveAgentMeAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveAgentMeAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/agentme-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/agentme-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/agentme-agent-custom");
