...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveAgentMeAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("agentme models auth setup-token")}\` or \`${formatCliCommand("agentme configure")}\`.`
          : `Run \`${formatCliCommand("agentme configure")}\` or set an API key env var.`;
