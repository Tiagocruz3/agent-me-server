...
import { readConfigFileSnapshot, type AgentMeConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: AgentMeConfig, model: string): AgentMeConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("agentme plugins install")}\`.`,
    );
  }
