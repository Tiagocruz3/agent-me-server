...
export async function updateConfig(
  mutator: (cfg: AgentMeConfig) => AgentMeConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: AgentMeConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: AgentMeConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("agentme agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
