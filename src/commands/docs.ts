...
const SEARCH_TOOL = "https://docs.agentme.ai/mcp.SearchAgentMe";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.agentme.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('agentme docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.agentme.ai/");
      runtime.log(`Search: ${formatCliCommand('agentme docs "your query"')}`);
    }
    return;
  }
