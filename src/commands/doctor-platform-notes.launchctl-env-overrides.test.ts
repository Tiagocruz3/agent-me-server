...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "AGENTME_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("AGENTME_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv AGENTME_GATEWAY_TOKEN");
    expect(message).not.toContain("AGENTME_GATEWAY_PASSWORD");
  });
