...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "agentme-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.AGENTME_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "agentme");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.AGENTME_STATE_DIR = stateDir;
    process.env.AGENTME_CONFIG_PATH = path.join(stateDir, "agentme.json");
