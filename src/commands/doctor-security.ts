...
import type { AgentMeConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: AgentMeConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("agentme security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("agentme configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("agentme config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("agentme doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("agentme config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
