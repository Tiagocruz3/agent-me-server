...
beforeEach(() => {
  ...
  resolveAgentMePackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.AGENTME_STATE_DIR;
  originalUpdateInProgress = process.env.AGENTME_UPDATE_IN_PROGRESS;
  process.env.AGENTME_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "agentme-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.AGENTME_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.AGENTME_UPDATE_IN_PROGRESS;
  } else {
    process.env.AGENTME_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveAgentMePackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadAgentMePlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/agentme-root.js", () => ({
  resolveAgentMePackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "agentme-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.AGENTME_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when AGENTME_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/agentme.json",
      exists: true,
      raw: "{}",
    ...
    process.env.AGENTME_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
