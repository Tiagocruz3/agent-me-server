...
function captureEnv(): EnvSnapshot {
  return {
    ...
    password: process.env.AGENTME_GATEWAY_PASSWORD,
    disableConfigCache: process.env.AGENTME_DISABLE_CONFIG_CACHE,
  };
}
...
function restoreEnv(prev: EnvSnapshot): void {
  ...
  restoreEnvVar("AGENTME_STATE_DIR", prev.stateDir);
  restoreEnvVar("AGENTME_CONFIG_PATH", prev.configPath);
  restoreEnvVar("AGENTME_SKIP_CHANNELS", prev.skipChannels);
  restoreEnvVar("AGENTME_SKIP_GMAIL_WATCHER", prev.skipGmail);
  restoreEnvVar("AGENTME_SKIP_CRON", prev.skipCron);
  restoreEnvVar("AGENTME_SKIP_CANVAS_HOST", prev.skipCanvas);
  restoreEnvVar("AGENTME_GATEWAY_TOKEN", prev.token);
  restoreEnvVar("AGENTME_GATEWAY_PASSWORD", prev.password);
  restoreEnvVar("AGENTME_DISABLE_CONFIG_CACHE", prev.disableConfigCache);
}
...
async function withOnboardEnv(
  ...
  process.env.AGENTME_DISABLE_CONFIG_CACHE = "1";
  ...
  const configPath = path.join(tempHome, "agentme.json");
  process.env.HOME = tempHome;
  process.env.AGENTME_STATE_DIR = tempHome;
  process.env.AGENTME_CONFIG_PATH = configPath;
  vi.resetModules();
...
describe("onboard (non-interactive): provider auth", () => {
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("agentme-onboard-xai-", async ({ configPath, runtime }) => {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("agentme-onboard-ai-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("agentme-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("agentme-onboard-openai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores Cloudflare AI Gateway API key and metadata", async () => {
    await withOnboardEnv("agentme-onboard-cf-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers Cloudflare auth choice from API key flags", async () => {
    await withOnboardEnv("agentme-onboard-cf-gateway-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
          cloudflareAiGatewayAccountId: "cf-account-id",
