...
const requireAgentDir = () => {
  const agentDir = process.env.AGENTME_AGENT_DIR;
  if (!agentDir) {
    throw new Error("AGENTME_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.AGENTME_STATE_DIR;
  const previousAgentDir = process.env.AGENTME_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.AGENTME_AGENT_DIR;
    } else {
      process.env.AGENTME_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.AGENTME_OAUTH_DIR;
  });

  it("writes auth-profiles.json under AGENTME_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "agentme-oauth-"));
    process.env.AGENTME_STATE_DIR = tempStateDir;
    process.env.AGENTME_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.AGENTME_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to AGENTME_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "agentme-minimax-"));
    process.env.AGENTME_STATE_DIR = tempStateDir;
    process.env.AGENTME_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.AGENTME_AGENT_DIR;
