...
function summarizeGroupPolicy(cfg: AgentMeConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function collectModels(cfg: AgentMeConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function hasWebSearchKey(cfg: AgentMeConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: AgentMeConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: AgentMeConfig): boolean {
...
function isBrowserEnabled(cfg: AgentMeConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: AgentMeConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: AgentMeConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep AGENTME_STATE_DIR on a local-only volume and re-run "${formatCliCommand("agentme security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: AgentMeConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer AGENTME_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(cfg: AgentMeConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: AgentMeConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: AgentMeConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
