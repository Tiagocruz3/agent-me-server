...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.agentmeCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.agentmeCanvasA2UIAction &&
        typeof window.agentmeCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.agentmeSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("agentme:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android AgentMe node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.agentmeSendUserAction === "function"
        ? window.agentmeSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.AGENTME_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.AGENTME_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>AgentMe Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
