...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("agentmeCanvasA2UIAction");
    expect(out).toContain("agentmeSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "agentme-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("agentmeSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.agentmeA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__agentme__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("agentme-a2ui-host");
      expect(html).toContain("agentmeCanvasA2UIAction");

      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/__agentme__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("agentmeA2UI");
    } finally {
      await server.close();
      if (createdBundle) {
