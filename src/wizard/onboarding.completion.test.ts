...
describe("setupOnboardingShellCompletion", () => {
  it("QuickStart: installs without prompting", async () => {
    ...
    const deps = {
      resolveCliName: () => "agentme",
      checkShellCompletionStatus: vi.fn(async () => ({
        shell: "zsh",
        profileInstalled: false,
        cacheExists: false,
        cachePath: "/tmp/agentme.zsh",
    ...
    expect(deps.ensureCompletionCacheExists).toHaveBeenCalledWith("agentme");
    expect(deps.installCompletion).toHaveBeenCalledWith("zsh", true, "agentme");
