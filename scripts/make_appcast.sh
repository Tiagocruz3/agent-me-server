...
ZIP=${1:?"Usage: $0 AgentMe-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/agentme/agentme/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^AgentMe-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/agentme/agentme/releases/download/v${VERSION}/"}
