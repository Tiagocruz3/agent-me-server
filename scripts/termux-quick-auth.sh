...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/agentme/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "AgentMe Re-Auth" -c "After getting key, run: ssh $SERVER '~/agentme/scripts/mobile-reauth.sh'" --id agentme-auth
        ;;
    *)
