...
BASE_IMAGE="${BASE_IMAGE:-agentme-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-agentme-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=agentme.sandbox=1)
NOTE
