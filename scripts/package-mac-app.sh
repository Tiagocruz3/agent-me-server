...
# Build and bundle AgentMe into a minimal .app we can open.
# Outputs to dist/AgentMe.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/AgentMe.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="AgentMe"
BUNDLE_ID="${BUNDLE_ID:-ai.agentme.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/agentme/agentme/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/AgentMe/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :AgentMeBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :AgentMeGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/AgentMe"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/AgentMe"
fi
chmod +x "$APP_ROOT/Contents/MacOS/AgentMe"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/AgentMe" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/AgentMe/Resources/AgentMe.icns" "$APP_ROOT/Contents/Resources/AgentMe.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/AgentMe/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying AgentMeKit resources"
AGENTMEKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/AgentMeKit_AgentMeKit.bundle"
if [ -d "$AGENTMEKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/AgentMeKit_AgentMeKit.bundle"
  cp -R "$AGENTMEKIT_BUNDLE" "$APP_ROOT/Contents/Resources/AgentMeKit_AgentMeKit.bundle"
else
  echo "WARN: AgentMeKit resource bundle not found at $AGENTMEKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running AgentMe"
killall -q AgentMe 2>/dev/null || true
