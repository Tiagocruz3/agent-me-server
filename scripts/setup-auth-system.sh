#!/bin/bash
# Setup AgentMe Auth Management System
...
echo "=== AgentMe Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'agentme-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  AgentMe message: Send warning via AgentMe itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/agentme-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/agentme-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/agentme-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now agentme-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status agentme-auth-monitor.timer"
echo ""
echo "Quick commands:"
