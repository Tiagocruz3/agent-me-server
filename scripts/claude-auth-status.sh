...
# Checks both Claude Code and AgentMe auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
AGENTME_AUTH="$HOME/.agentme/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    agentme models status --json 2>/dev/null || true
}
...
check_agentme_auth() {
    ...
    expires=$(jq -r '
        ...
        | max // 0
    ' "$AGENTME_AUTH" 2>/dev/null || echo "0")
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    agentme_status=$(check_agentme_auth 2>/dev/null || true)

    claude_expires=0
    agentme_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        agentme_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        agentme_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$AGENTME_AUTH" 2>/dev/null || echo "0")
    fi

    jq -n \
        --arg cs "$claude_status" \
        --arg ce "$claude_expires" \
        --arg bs "$agentme_status" \
        --arg be "$agentme_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            agentme: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$agentme_status" == EXPIRED* ]] || [[ "$agentme_status" == MISSING* ]]; then
        echo "AGENTME_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$agentme_status" == EXPIRING* ]]; then
        echo "AGENTME_EXPIRING"
        exit 2
    else
        echo "OK"
        exit 0
...
echo ""
echo "AgentMe Auth (~/.agentme/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$AGENTME_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$AGENTME_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'agentme doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active agentme >/dev/null 2>&1; then
    echo -e "AgentMe service: ${GREEN}running${NC}"
else
    echo -e "AgentMe service: ${RED}NOT running${NC}"
fi
