...
IMAGE_NAME="agentme-plugins-e2e"
...
echo "Running plugins Docker E2E..."
	docker run --rm -t "$IMAGE_NAME" bash -lc '
	  ...
	  if [ -f dist/index.mjs ]; then
	    AGENTME_ENTRY="dist/index.mjs"
	  elif [ -f dist/index.js ]; then
	    AGENTME_ENTRY="dist/index.js"
	  else
	    echo "Missing dist/index.(m)js (build output):"
	    ls -la dist || true
	    exit 1
	  fi
	  export AGENTME_ENTRY

	  home_dir=$(mktemp -d "/tmp/agentme-plugins-e2e.XXXXXX")
	  export HOME="$home_dir"
  mkdir -p "$HOME/.agentme/extensions/demo-plugin"

  cat > "$HOME/.agentme/extensions/demo-plugin/index.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
  cat > "$HOME/.agentme/extensions/demo-plugin/agentme.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$AGENTME_ENTRY" plugins list --json > /tmp/plugins.json
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/agentme-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@agentme/demo-plugin-tgz",
  "version": "0.0.1",
...
JS
  cat > "$pack_dir/package/agentme.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
JSON
  tar -czf /tmp/demo-plugin-tgz.tgz -C "$pack_dir" package
	  ...
	  node "$AGENTME_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
	  node "$AGENTME_ENTRY" plugins list --json > /tmp/plugins2.json
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/agentme-plugin-dir.XXXXXX")"
  cat > "$dir_plugin/package.json" <<'"'"'JSON'"'"'
{
  "name": "@agentme/demo-plugin-dir",
  "version": "0.0.1",
  "agentme": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$dir_plugin/agentme.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$AGENTME_ENTRY" plugins install "$dir_plugin"
	  node "$AGENTME_ENTRY" plugins list --json > /tmp/plugins3.json
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/agentme-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@agentme/demo-plugin-file",
  "version": "0.0.1",
  "agentme": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$file_pack_dir/package/agentme.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$AGENTME_ENTRY" plugins install "file:$file_pack_dir/package"
	  node "$AGENTME_ENTRY" plugins list --json > /tmp/plugins4.json
