...
IMAGE_NAME="agentme-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="agentme-net-e2e-$$"
GW_NAME="agentme-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "AGENTME_GATEWAY_TOKEN=$TOKEN" \
	  -e "AGENTME_SKIP_CHANNELS=1" \
	  -e "AGENTME_SKIP_GMAIL_WATCHER=1" \
	  -e "AGENTME_SKIP_CRON=1" \
	  -e "AGENTME_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
