...
#
# Suggested cron: */30 * * * * /home/admin/agentme/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send AgentMe notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., agentme-alerts)
...
STATE_FILE="$HOME/.agentme/auth-monitor-state"
...
send_notification() {
    ...
    # Send via AgentMe if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use agentme
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via AgentMe to $NOTIFY_PHONE..."
            agentme send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! AgentMe is down. Run: ssh l36 '~/agentme/scripts/mobile-reauth.sh'" "urgent"
    exit 1
