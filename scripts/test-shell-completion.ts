 ...
 * `agentme update`. Use it to verify the completion installation flow
...
\`agentme update\`, \`agentme doctor\`, and \`agentme onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# AgentMe Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`agentme completion --install\` later to enable.`));
    return;
  }
