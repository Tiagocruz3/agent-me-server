...
import { AgentMeApp } from "./app.ts";
import "../styles.css";

// oxlint-disable-next-line typescript/unbound-method
const originalConnect = AgentMeApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("agentme-app") as AgentMeApp;
...
beforeEach(() => {
  AgentMeApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__AGENTME_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  AgentMeApp.prototype.connect = originalConnect;
  window.__AGENTME_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/agentme/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/agentme");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/agentme/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__AGENTME_CONTROL_UI_BASE_PATH__ = "/agentme";
    const app = mountApp("/agentme/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/agentme");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/agentme/sessions");
  });
